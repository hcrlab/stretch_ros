...
from stretch_funmap.numba_manipulation_planning import numba_find_base_poses_that_reach_target, \
    numba_check_that_tool_can_deploy


def plan_surface_coverage(tool_start_xy_pix, tool_end_xy_pix, tool_extension_direction_xy_pix, step_size_pix,
                          max_extension_pix, surface_mask_image, obstacle_mask_image):
    ...
    for n in range(num_steps):
        ...
        first_surface_contact_xy, last_surface_contact_xy, first_obstacle_contact_xy = numba_find_line_path_on_surface(
            np.int32(np.round(start_xy_pix)), end_xy_pix, surface_mask_image, obstacle_mask_image)
...
def detect_cliff(image, m_per_pix, m_per_height_unit, robot_xy_pix, display_text='', display_images=False):
    ...
    lines = cv2.HoughLinesP(canny_edges, rho, angular_resolution, threshold, minLineLength=minLineLength,
...
class ManipulationView():
    def __init__(self, tf2_buffer, debug_directory=None):
        ...
        voi_origin = np.array(
            [-(voi_side_x_m / 2.0), -(voi_side_y_m + robot_right_edge_m), -lowest_distance_below_ground])
    ...
    def estimate_reach_to_contact_distance(self, tooltip_frame, tf2_buffer, save_debugging_images=True):
        ...
        if save_debugging_images and (self.debug_directory is not None):
            ...
            rospy.loginfo(
                'ManipulationView estimate_reach_to_contact_distance: No debug directory provided, so debugging data will not be saved.')
    ...
    def get_pregrasp_planar_translation(self, grasp_target, tf2_buffer):
        ...
        if debug and (self.debug_directory is not None):
            ...
            cv2.circle(rgb_image, tuple(np.int32(np.round(grasp_target['location_xy_pix']))), radius, [0, 0, 255],
                       width)
    ...
    def get_surface_wiping_plan(self, tf2_buffer, tool_width_m, tool_length_m, step_size_m):
        ...
        if self.updated:
            ...
            if surface_mask is not None:
                ...
                strokes = plan_surface_coverage(tool_start_xy_pix, tool_end_xy_pix, tool_extension_direction_xy_pix,
                                                step_size_pix, max_extension_pix, surface_mask, obstacle_mask)
                ...
                if debug and (self.debug_directory is not None):
                    ...
                    line_width = 1
                    for n, b, [s, e] in strokes:
                        cv2.line(rgb_image, tuple(np.int32(np.round(s))), tuple(np.int32(np.round(e))), [255, 0, 0],
                                 line_width)
                    if len(strokes) > 0:
                        first_unextended_target = strokes[0][1]
                        radius = 5
                        width = 2
                        cv2.circle(rgb_image, tuple(np.int32(np.round(first_unextended_target))), radius, [0, 255, 0],
                                   width)
...
class ManipulationPlanner:
    ...
    def base_pose(self, max_height_image, target_xyz_pix, robot_xya_pix, image_display_on=True):
        ...
        # and and the map.
        distance_map, traversable_mask = sm.process_max_height_image(max_height_image, robot_x_pix, robot_y_pix,
                                                                     robot_ang_rad, display_on=False)

        # Dilate finger obstacles to account for the gripper and arm
        # widths. This should also reduce issues due to undersampling by
        # the radial search.
        reach_width_m = self.planar_model.gripper_and_arm_width_m + (
                    2.0 * self.planar_model.gripper_and_arm_width_safety_margin_m)
        ...
        yaw_offset_left_m = (
                                        self.planar_model.gripper_and_arm_width_m / 2.0) - self.planar_model.radius_of_yaw_cylinder_m
        ...
        # reach the target.
        base_xy_image, base_ang_image, arm_reach_image = numba_find_base_poses_that_reach_target(target_x, target_y,
                                                                                                 num_angles, pix_per_m,
                                                                                                 start_distance_m,
                                                                                                 self.planar_model.max_arm_travel_m,
                                                                                                 origin_offset_left_m,
                                                                                                 origin_offset_length_m,
                                                                                                 obstacle_image)
        if close_ray_plan:
        ...
        if image_display_on:
            ...
            if image_display_on:
                ...
                base_ang_image_uint8 = np.uint8(255.0 * ((base_ang_image - np.min(base_ang_image)) / (
                            np.max(base_ang_image) - np.min(base_ang_image))))
