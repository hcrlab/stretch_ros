...
@njit(fastmath=True)
def numba_compare_images_2(image_to_warp, target_image, target_image_not_smoothed, affine_matrix, m_per_height_unit,
                           match_threshold_m=0.07, weight_by_height=True):
    ...
    if (target_image_height != target_image_not_smoothed_height) or (
            target_image_width != target_image_not_smoothed_width):
        print(
            'ERROR: numba_compare_images_2: target_image and target_image_not_smoothed must have the same dimensions, yet')
        print('       target_image_not_smoothed_width, target_image_not_smoothed_height =',
              (target_image_not_smoothed_width, target_image_not_smoothed_height))
    ...
    for to_warp_y in range(image_to_warp_height):
        ...
        for to_warp_x in range(image_to_warp_width):
            ...
            if to_warp_value != 0:
                ...
                if (target_x >= 0) and (target_x < target_image_width) and (target_y >= 0) and (
                        target_y < target_image_height):
