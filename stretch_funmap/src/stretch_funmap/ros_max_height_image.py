...
class ROSVolumeOfInterest(VolumeOfInterest):
    ...
    def from_serialization(self, data):
        d = data
        voi = ROSVolumeOfInterest(d['frame_id'], np.array(d['origin']), np.array(d['axes']), d['x_in_m'], d['y_in_m'],
                                  d['z_in_m'])
        return voi
    ...
    def get_points_to_voi_matrix_with_tf2(self, points_frame_id, tf2_buffer, lookup_time=None, timeout_s=None):
        ...
        try:
            ...
            rospy.logwarn(
                'ROSVolumeOfInterest.get_points_to_voi_matrix_with_tf2: failed to lookup transform. self.frame_id = {0}, points_frame_id = {1}, lookup_time = {2}, timeout_s = {3}'.format(
                    self.frame_id, points_frame_id, lookup_time, timeout_s))
            return None, None
...
class ROSMaxHeightImage(MaxHeightImage):
    ...
    def from_file(self, base_filename):
        ...
        max_height_image = ROSMaxHeightImage(voi, m_per_pix, image.dtype, m_per_height_unit,
    ...
    def get_points_to_image_mat(self, ros_frame_id, tf2_buffer, lookup_time=None, timeout_s=None):
        ...
        if points_to_voi_mat is not None:
            ...
            if np.issubdtype(dtype, np.integer):
                ...
                rospy.logerr(
                    'ROSMaxHeightImage.get_points_to_image_mat: unsupported image type used for max_height_image, dtype = {0}'.format(
                        dtype))
                assert (False)
    ...
    def get_image_to_points_mat(self, ros_frame_id, tf2_buffer, lookup_time=None, timeout_s=None):
        ...
        if voi_to_points_mat is not None:
            ...
            if np.issubdtype(dtype, np.integer):
                ...
                rospy.logerr(
                    'ROSMaxHeightImage.get_image_to_points_mat: unsupported image type used for max_height_image, dtype = {0}'.format(
                        dtype))
                assert (False)
    ...
    def make_robot_footprint_unobserved(self, robot_x_pix, robot_y_pix, robot_ang_rad):
        ...
        if self.camera_depth_image is not None:
            na.draw_robot_footprint_rectangle(robot_x_pix, robot_y_pix, robot_ang_rad, self.m_per_pix,
                                              self.camera_depth_image, value=0)
        if self.rgb_image is not None:
            na.draw_robot_footprint_rectangle(robot_x_pix, robot_y_pix, robot_ang_rad, self.m_per_pix, self.rgb_image,
                                              value=0)
    ...
    def make_robot_mast_blind_spot_unobserved(self, robot_x_pix, robot_y_pix, robot_ang_rad):
        ...
        na.draw_robot_mast_blind_spot_wedge(robot_x_pix, robot_y_pix, robot_ang_rad, self.m_per_pix, self.image,
                                            value=0)
        if self.camera_depth_image is not None:
            na.draw_robot_mast_blind_spot_wedge(robot_x_pix, robot_y_pix, robot_ang_rad, self.m_per_pix,
                                                self.camera_depth_image, value=0)
        if self.rgb_image is not None:
            na.draw_robot_mast_blind_spot_wedge(robot_x_pix, robot_y_pix, robot_ang_rad, self.m_per_pix, self.rgb_image,
                                                value=0)
    ...
    def from_points_with_tf2(self, points, points_frame_id, tf2_buffer, points_timestamp=None, timeout_s=None):
        ...
        if points_to_voi_mat is not None:
            ...
            rospy.logwarn(
                'ROSMaxHeightImage.from_points_with_tf2: failed to update the image likely due to a failure to lookup the transform using TF2. points_frame_id = {0}, points_timestamp = {1}, timeout_s = {2}'.format(
                    points_frame_id, points_timestamp, timeout_s))
    ...
    def from_rgb_points_with_tf2(self, rgb_points, points_frame_id, tf2_buffer, points_timestamp=None, timeout_s=None):
        ...
        if points_to_voi_mat is not None:
            ...
            rospy.logwarn(
                'ROSMaxHeightImage.from_rgb_points_with_tf2: failed to update the image likely due to a failure to lookup the transform using TF2. points_frame_id = {0}, points_timestamp = {1}, timeout_s = {2}'.format(
                    points_frame_id, points_timestamp, timeout_s))
