...
def find_object_to_grasp(height_image, display_on=False):
    ...
    if display_on:
        rgb_image = height_image.rgb_image.copy()
        color_label_image = sk.color.label2rgb(label_image, image=rgb_image, colors=None, alpha=0.3, bg_label=0,
...
def find_closest_flat_surface(height_image, robot_xy_pix, display_on=False):
    ...
    segments_image, segment_info, height_to_segment_id = segment(image, m_per_unit, zero_height, segmentation_scale,
                                                                 verbose=False)
    ...
    if display_on:
        color_label_image = sk.color.label2rgb(label_image, image=image_rgb, colors=None, alpha=0.3, bg_label=0,
                                               bg_color=(0, 0, 0), image_alpha=1, kind='overlay')
    ...
    if display_on:
        surface_label_image_rgb = sk.color.label2rgb(surface_label_image, image=None, colors=None, alpha=0.3,
...
def visualize_hist(value_hist, segments_image, height_to_segment_id, bins_to_mark=[]):
    ...
    if np.sum(height_to_segment_id) > 0:
        height_to_segment_id_image = draw_histogram(height_to_segment_id, hist_image_w, hist_image_h, border=20,
                                                    bins_to_mark=bins_to_mark)
...
def histogram_segment(segments_image, image,
    ...
    for min_index in min_bins:
        ...
        if (left_segment_id is not None) and (left_segment_id != 0):
            ...
            if (right_segment_id is not None) and (right_segment_id != 0):
                ...
                # would ever happen.
                print(
                    'WARNING: histogram_segment - no valid bump segment was found to the left or right of a relative minumum. This may be due to a bug. It also might result in a height region being unassigned to a segment.')
    ...
    if not np.alltrue(all_segments > 0):
        print(
            'WARNING: histogram_segment - all_segments has unasigned height bins. This could be due to a bug and is undesirable for a height segmentation.')
        print('         all_segments =', all_segments)
    ...
    for c in bin_segments:
        ...
        if max_value <= epsilon:
            ...
            # corresponds.
            segment_info[segment_id] = {'bin_index': max_bin, 'bin_value': value_hist[max_bin],
                                        'height_unit': best_value}
...
def segment(image, m_per_unit, zero_height_m, segmentation_scale=0.1, verbose=False):
    ...
    value_hist, bin_edges = np.histogram(values, bins=num_bins, range=(range_min, range_max), normed=False,
...
def full_segment(image, image_rgb, segmentation_scale, m_per_unit, zero_height=0.0, visualize=True, visualize_title='',
                 verbose=False):
    segments_image, segment_info, height_to_segment_id = segment(image, m_per_unit, zero_height, segmentation_scale,
                                                                 verbose)
    ...
    if image_rgb is None:
        color_label_image = sk.color.label2rgb(label_image, image=None, colors=None, alpha=0.3, bg_label=0,
                                               bg_color=(0, 0, 0), image_alpha=1, kind='overlay')
    else:
        color_label_image = sk.color.label2rgb(label_image, image=image_rgb, colors=None, alpha=0.3, bg_label=0,
                                               bg_color=(0, 0, 0), image_alpha=1, kind='overlay')
...
def find_floor(segment_info, segments_image, verbose=False):
    ...
    for i in segment_info:
        ...
        if verbose:
            print(
                'segment = {0}, histogram bin_value = {1}, height_m = {2}, min_floor_m = {3}, max_floor_m = {4}'.format(
                    i, bin_value, height_m, min_floor_m, max_floor_m))
...
def process_max_height_image(max_height_im, robot_x_pix, robot_y_pix, robot_ang_rad, display_on=False):
    ...
    if floor_id is not None:
        ...
        distance_map, traversable_mask = na.distance_map_simple(floor_mask, m_per_pix, min_robot_width_m, robot_x_pix,
                                                                robot_y_pix, robot_ang_rad, display_on=display_on,
                                                                verbose=True)
